...
//RST ->PA12
#define PIN_TFT_RST_High()	GPIOA->BSRR = GPIO_Pin_12
#define PIN_TFT_RST_Low()	GPIOA->BRR  = GPIO_Pin_12
//DC  ->PA11
#define PIN_TFT_DC_Data()	GPIOA->BSRR = GPIO_Pin_11
#define PIN_TFT_DC_Cmd()	GPIOA->BRR  = GPIO_Pin_11
//CS  ->PA8
#define PIN_TFT_CS_High()	GPIOA->BSRR = GPIO_Pin_8
#define PIN_TFT_CS_Low()	GPIOA->BRR  = GPIO_Pin_8
...
static void Init_TFTPin(void)
{
	...
	GPIO_InitStruct.GPIO_Pin = GPIO_Pin_0|GPIO_Pin_13|GPIO_Pin_15;
...
void Init_TFT(void)
{
	...
	white = ~white;
	pink = ~pink;
	TFT_SetRect(25+0,0,width,160);
	for(int i=0;i<width*160;i++)
	{
		TFT_WriteData16(pink);
	}
	TFT_SetRect(25+width,0,width,160);
	for(int i=0;i<width*160;i++)
	{
		TFT_WriteData16(white);
	}
	TFT_SetRect(25+width*2,0,width,160);
	for(int i=0;i<width*160;i++)
	{
